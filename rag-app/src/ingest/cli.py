...
def ingest(directory_path):
    ...
    def ingestion_failed():
        logger.info("❌ Ingestion failed, please re-run the ingestion again, If the issue still persists, please report an issue.")
    ...
    emb_model_dict, llm_model_dict, vlm_model_dict, _ = get_model_endpoints()
    ...
    converted_files, converted_pdf_stats = extract_document_data(
        ...
        vlm_model_dict['vlm_model'], vlm_model_dict['vlm_endpoint'])
    if not converted_files:
        ingestion_failed()
        return
    logger.debug(f"Converted files: {converted_files}")

    original_filenames, input_txt_files, input_tab_files, input_img_files = get_txt_tab_img_filenames(converted_files, out_path)
    ...
    for in_chunk_f, in_tab_f, in_img_f, orig_fn in zip(chunked_files, input_tab_files, input_img_files, original_filenames):
        # Combine all chunks (text, image summaries, table summaries)
        filtered_chunks = create_chunk_documents(
            in_chunk_f, in_tab_f, in_img_f, orig_fn, out_path, collection_name)
    ...
    if len(unprocessed_files):
        logger.info("Ingestion completed partially, please re-run the ingestion again to ingest the following files.\n" + '\n'.join(unprocessed_files) + "\nIf the issue still persists, please report an issue.")
    else:
    ...
    total_images = sum(converted_pdf_stats[file]["image_count"] for file in converted_pdf_stats)

    header_format = f"| {'PDF':<{max_file_len}} | {'Total Pages':^{15}} | {'Total Tables':>{15}} | {'Total Images':>{15}} |"
    print("-" * len(header_format))
    print(header_format)
    print("-" * len(header_format))
    for file in converted_pdf_stats:
        print(f"| {file:<{max_file_len}} | {converted_pdf_stats[file]['page_count']:^{15}} | {converted_pdf_stats[file]['table_count']:>{15}} | {converted_pdf_stats[file]['image_count']:>{15}} |")
    print("-" * len(header_format))
    print(f"| {'Total':<{max_file_len}} | {total_pages:^{15}} | {total_tables:>{15}} | {total_images:>{15}} |")
    print("-" * len(header_format))
...
def main(argv=None):
    """
    Entrypoint for CLI. `argv` may be passed (list of args) for programmatic invocation;
    when argv is None argparse reads from sys.argv (normal CLI behavior).
    """
    # parse args (argv=None means parse from sys.argv)
    command_args = parser.parse_args(argv)

    # configure log level (priority: CLI flag, then env var)
...
# Ensure the CLI only runs when module executed as script, not on import.
